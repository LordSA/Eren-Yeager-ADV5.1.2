...
from info import AUTH_USERS, LOG_CHANNEL   # Make sure LOG_CHANNEL is set in info.py

OWNER_ID = AUTH_USERS  # Your user ID(s)
...
async def set_variable(client: Client, message: Message):
    if len(message.command) < 3:
        return await message.reply_text("⚠️ Usage: <code>/setvar VAR_NAME value</code>")
...
async def get_variable(client: Client, message: Message):
    if len(message.command) != 2:
        return await message.reply_text("⚠️ Usage: <code>/getvar VAR_NAME</code>")
...
async def delete_variable(client: Client, message: Message):
    if len(message.command) != 2:
        return await message.reply_text("⚠️ Usage: <code>/delvar VAR_NAME</code>")
